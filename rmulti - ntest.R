...
delta <- c(0.95)
theta1 <- 0
lcbdata <- matrix(data = NA, nrow = 8, ncol = 5, dimnames = list(c("n=1","n=2","n=4","n=8","n=16","n=32","n=64","n=128"),c("d=0,95","d=0,9","d=0,75","d=0,5","d=0,25")))
slcbdata <- matrix(data = NA, nrow = 8, ncol = 5, dimnames = list(c("n=1","n=2","n=4","n=8","n=16","n=32","n=64","n=128"),c("d=0,95","d=0,9","d=0,75","d=0,5","d=0,25")))
for(po in 0:7){
  
  print(po)
  jobs <- c(2^po,rep(2,1))
  maxjobs <- max(jobs)
  z <- log(jobs[2])/log(maxjobs)
  a <- c(NaN,rep(1+10^-15,1))
  omega <- c(NaN,rep(10^-15,1))
  pexp <- pgamma(1,1+10^-15,rate = 10^-15)
  theta1 <- qgamma((1-pexp)*z*delta[1] + pexp,1+10^-15,rate = 10^-15) - 100
...
for (i in 1:runs){
  ...
  for(k in 1:length(delta)){
    ...
    for(j in 1:sum(jobs)){
      ...
      if(bestclass > 1){
        ...
        lcbMeasure <- round(lcbMeasure,20)
      }
  ...
  for(k in 1:length(delta)){
    ...
    slcbMeasure <- round(slcbMeasure,30)
    for(j in 1:sum(jobs)){
...
lcbdata[po+1,] <- (lcbscore/lseptscore)
slcbdata[po+1,] <- (slcbscore/lseptscore)
}
