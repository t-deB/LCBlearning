...
lcbdata <- matrix(data = NA, nrow = 7, ncol = 8, dimnames = list(c(),c()))
slcbdata <- matrix(data = NA, nrow = 7, ncol = 8, dimnames = list(c(),c()))
for(po in 1:7){
  poa <- 2^po
  for(pa in 0:7){
    poo = 2^pa
    print(po)
    jobs <- c(rep(poo,poa))
    a <- c(NaN,1002,rep(1+10^-9,poa-2))
    omega <- c(NaN,1000,rep(10^-9 + 10^-10,poa-2))
    theta1 <- 1
  ...
  for(k in 1:length(delta)){
    ...
    for(j in 1:sum(jobs)){
      ...
      if(bestclass > 1){
        ...
        slcbMeasure[bestclass] <- qgamma(z[bestclass]*delta*(1-pexp) + pexp,slcba[bestclass],rate = slcbomega[bestclass])
        slcbMeasure <- round(slcbMeasure,15)
      }
...
lcbdata[po,pa+1] <- (lcbscore[1]/lseptscore)
slcbdata[po,pa+1] <- (slcbscore[1]/lseptscore)
  }
}
